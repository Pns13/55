...
def passwrd():
	...
	print('')
	print(f'➥➥➣Hasil {h}OK{u} Disimpan Di : {h}OK/%s {k}'%(okc))
	print(f'➥➥➣Hasil {k}CP{h} Disimpan Di : {k}CP/%s {k}'%(cpc))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('[bold red]       MAAF ANDA BELUM BERUNTUNG[bold red]')) 
				...
				cetak(nel('[bold purple]    SELAMAT ANDA SEDANG HOKI[bold purple]')) 
				print(f'\r{K}>> {idf}|{pw}{N}')     
